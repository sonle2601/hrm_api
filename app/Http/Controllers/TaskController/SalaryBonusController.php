...
class SalaryBonusController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($store) {
            $storeId = $store->id;
            $salaryBonus = SalaryBonus::where('store_id', $storeId)
    ...
    public function showSalaryBonusEmployee(Request $request,String $employeeId,string $startDate,String $endDate )
    {
        $token = $request->bearerToken();
        $manage_id = JWTAuth::getPayload($token)->get('sub');
    ...
    public function showSalaryBonusPersonal(Request $request,string $startDate,String $endDate )
    {
        $token = $request->bearerToken();
        ...
        $employeeId = $employee->id;
            ...
            $salaryBonus = SalaryBonus::where('employee_id', $employeeId)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->with('employee.information')
            ->get();
